...
class dLSM_EXPORT TableBuilder {
 public:
  ...
  virtual void FinishDataIndexBlock(BlockBuilder* block, BlockHandle* handle,
                                    ...
                                    size_t& block_size) = 0;
  virtual void FinishFilterBlock(FullFilterBlockBuilder* block,
                                 BlockHandle* handle,
                                 CompressionType compressiontype,
                                 size_t& block_size) = 0;
 ...
 protected:
  ...
  //      local_index_mr[0]); if (type_ == IO_type::Compact){
  ...
  //    // second, no buffer is outstanding, those two status will both have
  //    start - end = 1 bool data_inuse_empty = true; std::vector<ibv_mr*>
  //    local_index_mr; std::vector<ibv_mr*> local_filter_mr;
  ...
  //    // blocks.
  //    //
  //    // Invariant: r->pending_index_filter_entry is true only if data_block
  //    is empty. bool pending_index_filter_entry; BlockHandle
  //    pending_data_handle;  // Handle to add to index block
  //
