...
Block::~Block() {
  if (RDMA_Regiested) {
    ...
    //    No need to deallocate the data block from RDMA memory allocator since
    //    it is thread local.
    if (type_ == DataBlock) {
    ...
    if (type_ == IndexBlock &&
        rdma_mg_->Deallocate_Local_RDMA_Slot((void*)data_, IndexChunk)) {
      //      printf("Index Block RDMA registered memory deallocated
      //      successfull\n");
      return;
    }
    if (type_ == FilterBlock &&
        rdma_mg_->Deallocate_Local_RDMA_Slot((void*)data_, FilterChunk)) {
      //      printf("Block RDMA registered memory deallocated successfull\n");
      return;
    }
