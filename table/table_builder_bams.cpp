...
TableBuilder_BAMS::TableBuilder_BAMS(const Options& options, IO_type type,
                                     std::shared_ptr<RDMA_Manager> rdma_mg)
...
void TableBuilder_BAMS::Add(const Slice& key, const Slice& value) {
  ...
  if ((r->offset - r->offset_last_flushed + key.size() + value.size() +
       2 * sizeof(uint32_t)) > r->local_data_mr->length) {
...
void TableBuilder_BAMS::FinishDataBlock(BlockBuilder* block,
                                        BlockHandle* handle,
...
void TableBuilder_BAMS::FinishFilterBlock(FullFilterBlockBuilder* block,
                                          BlockHandle* handle,
                                          CompressionType compressiontype,
                                          size_t& block_size) {
  Rep* r = rep_;
...
void TableBuilder_BAMS::get_dataindexblocks_map(
...
size_t TableBuilder_BAMS::get_numentries() { return rep_->num_entries; }
}  // namespace dLSM