...
// TOFIX : now we suppose the index and filter block will not over the write
// buffer.
...
struct TableBuilder_ComputeSide::Rep {
  ...
  // second, no buffer is outstanding, those two status will both have start -
  // end = 1
  ...
  // blocks.
  //
  // Invariant: r->pending_index_filter_entry is true only if data_block is
  // empty.
...
//     return Status::InvalidArgument("changing comparator while building
//     table");
//   }
//
...
//   return Status::OK();
// }
// TODO: make it create a block every blocksize, flush every 1M. When flushing
// do not poll completion
...
void TableBuilder_ComputeSide::Add(const Slice& key, const Slice& value) {
  ...
  // How to predict the datasize before actually serilizae the data, so that
  // there will not be buffer overflow. First, predict whether the block will be
  // full
  // *   if so then finish the old data to a block make it insert to a new block
  // *   Second, if new block finished, check whether the write buffer can hold
  // a new block size.
  // *           if not, the flush temporal buffer content to the remote memory.
  const size_t estimated_block_size = r->data_block->CurrentSizeEstimate();
  // maximize added length to the block is key size + value size + restart point
  // + shared, nonshared, valuesize
  if (estimated_block_size + key.size() + value.size() + sizeof(size_t) +
          3 * sizeof(uint32_t) + kBlockTrailerSize >=
      r->options.block_size) {
    UpdateFunctionBLock();
    if (r->local_data_mr[0]->length - (r->offset - r->offset_last_flushed) <
        r->options.block_size) {
      FlushData();
    }
  }
  ...
  //  when write a index entry, the data block offset and data block size will
  //  be attached
  if (r->pending_index_filter_entry) {
    ...
    if (r->index_block->CurrentSizeEstimate() + r->last_key.size() +
            handle_encoding.size() + sizeof(uint32_t) + kBlockTrailerSize >
        r->local_index_mr[0]->length) {
      assert(false);
      BlockHandle dummy_handle;
      size_t msg_size;
      FinishDataIndexBlock(r->index_block, &dummy_handle,
...
void TableBuilder_ComputeSide::UpdateFunctionBLock() {
  ...
  FinishDataBlock(r->data_block, &r->pending_data_handle,
...
// difference is whether update the offset which will record the size of the
// data block. And the filter blocks has a different way to reset the block.
void TableBuilder_ComputeSide::FinishDataBlock(
    BlockBuilder* block, BlockHandle* handle, CompressionType compressiontype) {
  ...
  // TOTHINK: temporally disable the compression, because it can increase the
  // latency but it could
  // increase the available bandwidth. THis part depends on whether the
  // in-memory write can catch up with the high RDMA bandwidth.
  ...
  // #endif
  handle->set_offset(
      r->offset);  // This is the offset of the begginning of this block.
  ...
  if (r->status.ok()) {
    ...
    crc = crc32c::Extend(crc, trailer,
                         1);  // Extend crc to cover block compressiontype
...
void TableBuilder_ComputeSide::FinishDataIndexBlock(
    BlockBuilder* block, BlockHandle* handle, CompressionType compressiontype,
    size_t& block_size) {
  // File format contains a sequence of blocks where each block has:
  //    block_data: uint8[n]
...
void TableBuilder_ComputeSide::FinishFilterBlock(
    FullFilterBlockBuilder* block, BlockHandle* handle,
    CompressionType compressiontype, size_t& block_size) {
  Rep* r = rep_;
  block->Finish();
...
// TODO make flushing flush the data to the remote memory flushing to remote
// memory
void TableBuilder_ComputeSide::FlushData() {
  ...
  //  I was thinking that the start represent the oldest outgoing buffer, while
  //  the end is the latest buffer. When polling a result, the start index will
  //  moving forward and the start index will be changed to 0 when meeting the
  //  end of this index. If the start index meet the end index, then we need to
  //  allocate a new buffer, and insert to the local buffer vector. the end
  //  index will move forward because a new buffer will be in used, the start
  ...
  if (r->data_inuse_start == -1) {
    ...
    // check the maximum outstanding buffer number, if not set it the flushing
    // and compaction will be messed up
    //    int maximum_poll_number = r->data_inuse_end - r->data_inuse_start + 1
    //    >= 0 ?
    //                      r->data_inuse_end - r->data_inuse_start + 1:
    //                      (int)(r->local_data_mr.size()) - r->data_inuse_start
    //                      + r->data_inuse_end +1;
    ...
    r->data_inuse_end = r->data_inuse_end == r->local_data_mr.size() - 1
                            ? 0
                            : r->data_inuse_end + 1;
    ...
    if (r->data_inuse_start - r->data_inuse_end == 1 ||
        r->data_inuse_end - r->data_inuse_start ==
            r->local_data_mr.size() - 1) {
      ibv_mr* new_local_mr = new ibv_mr();
      rdma_mg->Allocate_Local_RDMA_Slot(*new_local_mr, FlushBuffer);
      if (r->data_inuse_start == 0) {  // if start is 0 then the insert will
                                       // also increase the index of end.
        r->data_inuse_end++;
      }
      // insert new mr at start while increase start by 1.
      r->local_data_mr.insert(r->local_data_mr.begin() + r->data_inuse_start++,
                              new_local_mr);
  ...
  //    }
  // #endif
  //    r->remote_data_mrs.insert({r->remote_data_mrs.rbegin()->first+1,
  //    remote_mr});
  //  }
  if (r->remote_data_mrs.empty()) {
  ...
  // Move the datablock pointer to the start of the next write buffer, the other
  // state of the data_block has already reseted before
  int next_buffer_index = r->data_inuse_end == r->local_data_mr.size() - 1
                              ? 0
                              : r->data_inuse_end + 1;

  assert(next_buffer_index != r->data_inuse_start);
  r->data_block->Move_buffer(const_cast<const char*>(
      static_cast<char*>(r->local_data_mr[next_buffer_index]->addr)));
  ...
  //  r->local_data_mr.size()); DEBUG_arg("MR element number is %lu\n",
  //  r->remote_data_mrs.size()); assert(r->data_inuse_start!=
  //  r->data_inuse_end);
  // No need to record the flushing times, because we can check from the remote
  // mr map element number.
}
void TableBuilder_ComputeSide::FlushDataIndex(size_t msg_size) {
  ...
  if (r->remote_dataindex_mrs.empty()) {
    ...
    r->remote_dataindex_mrs.insert(
        {r->remote_dataindex_mrs.rbegin()->first + 1, remote_mr});
  }
  // TOFIX: the index may overflow and need to create a new index write buffer,
  // otherwise
...
void TableBuilder_ComputeSide::FlushFilter(size_t& msg_size) {
  ...
  if (r->remote_filter_mrs.empty()) {
    ...
    r->remote_filter_mrs.insert(
        {r->remote_filter_mrs.rbegin()->first + 1, remote_mr});
  }
  // TOFIX: the index may overflow and need to create a new index write buffer,
  // otherwise
  //  it would be overwrited.
...
Status TableBuilder_ComputeSide::Finish() {
  ...
  // Write filter block
  // TODO:
  // i found that it could have many bloom filters for each data block but there
  // will be only one filter block. the result will append many bloom filters.
  ...
  if (ok() && r->filter_block != nullptr) {
    ...
    size_t msg_size;
    FinishFilterBlock(r->filter_block, &filter_block_handle, kNoCompression,
                      msg_size);
...
uint64_t TableBuilder_ComputeSide::NumEntries() const {
  return rep_->num_entries;
}

uint64_t TableBuilder_ComputeSide::FileSize() const { return rep_->offset; }
void TableBuilder_ComputeSide::get_datablocks_map(
    std::map<uint32_t, ibv_mr*>& map) {
  map = rep_->remote_data_mrs;
}
void TableBuilder_ComputeSide::get_dataindexblocks_map(
    std::map<uint32_t, ibv_mr*>& map) {
  map = rep_->remote_dataindex_mrs;
}
void TableBuilder_ComputeSide::get_filter_map(
    std::map<uint32_t, ibv_mr*>& map) {
  map = rep_->remote_filter_mrs;
}
size_t TableBuilder_ComputeSide::get_numentries() { return rep_->num_entries; }
