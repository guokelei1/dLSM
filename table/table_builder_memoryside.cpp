...
struct TableBuilder_Memoryside::Rep {
  ...
  // TODO: garbage collect all the local unused MR when destroyingnthis table
  // builder.
...
TableBuilder_Memoryside::TableBuilder_Memoryside(
    ...
    : rep_(
          new TableBuilder_Memoryside::Rep(options, type, std::move(rdma_mg))) {
...
TableBuilder_Memoryside::~TableBuilder_Memoryside() {
  ...
  rep_->rdma_mg->Deallocate_Local_RDMA_Slot(rep_->local_data_mr->addr,
                                            FlushBuffer);
  rep_->rdma_mg->Deallocate_Local_RDMA_Slot(rep_->local_index_mr->addr,
                                            FlushBuffer);
  rep_->rdma_mg->Deallocate_Local_RDMA_Slot(rep_->local_filter_mr->addr,
                                            FlushBuffer);
...
void TableBuilder_Memoryside::Add(const Slice& key, const Slice& value) {
  ...
  if (estimated_block_size + key.size() + value.size() + sizeof(size_t) +
    ...
    UpdateFunctionBLock();
    if (r->local_data_mr->length - (r->offset - r->offset_last_flushed) <
        r->options.block_size) {
      FlushData();
    }
  }
  ...
  if (r->pending_index_filter_entry) {
    ...
    if (r->filter_block != nullptr) {
      //      if (r->filter_block->CurrentSizeEstimate() + kBlockTrailerSize >
      //      r->local_filter_mr[0]->length){
      //        // Tofix: Finish itself contain Reset and Flush, Modify the
      //        filter block make
      //        // it update the r->offset and add crc and make filter block
      //        compatible with Finish block. BlockHandle dummy_handle; size_t
      //        msg_size; FinishFilterBlock(r->filter_block, &dummy_handle,
      //        kNoCompression, msg_size); FlushFilter(msg_size);
      //        r->filter_block->RestartBlock(r->offset);// put here for full
      //        filter
      //      }

      //      r->filter_block->StartBlock(r->offset); // put here for block
      //      based filter
    }
...
void TableBuilder_Memoryside::FinishDataBlock(BlockBuilder* block,
                                              BlockHandle* handle,
...
void TableBuilder_Memoryside::FinishDataIndexBlock(
...
void TableBuilder_Memoryside::FinishFilterBlock(FullFilterBlockBuilder* block,
                                                BlockHandle* handle,
                                                CompressionType compressiontype,
                                                size_t& block_size) {
  Rep* r = rep_;
...
uint64_t TableBuilder_Memoryside::NumEntries() const {
  return rep_->num_entries;
}

uint64_t TableBuilder_Memoryside::FileSize() const { return rep_->offset; }
void TableBuilder_Memoryside::get_datablocks_map(
    std::map<uint32_t, ibv_mr*>& map) {
  map = rep_->local_data_mrs;
}
void TableBuilder_Memoryside::get_dataindexblocks_map(
    std::map<uint32_t, ibv_mr*>& map) {
...
size_t TableBuilder_Memoryside::get_numentries() { return rep_->num_entries; }
}  // namespace dLSM