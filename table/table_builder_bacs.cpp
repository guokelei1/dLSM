...
void TableBuilder_BACS::Add(const Slice& key, const Slice& value) {
  ...
  if ((r->offset - r->offset_last_flushed + key.size() + value.size() +
       2 * sizeof(uint32_t)) > r->local_data_mr[0]->length) {
    FlushData();  // reset the buffer inside
  }
  //  const size_t estimated_block_size = r->data_block->CurrentSizeEstimate();
  //  if (estimated_block_size + key.size() + value.size() +sizeof(size_t) +
  //  kBlockTrailerSize >= r->options.block_size) {
  //
  //  }

  // Create a new index entry for every items.
  {
    // Add to index block from the second data, and when finishing the index
    // block add the smallest successor of the last key.
    std::string handle_encoding;
    // This index point to this offset and the key is this key.
    r->pending_data_handle.set_offset(
        r->offset);  // This is the offset of the begginning of this block.
    r->pending_data_handle.set_size(key.size() + value.size() +
                                    2 * sizeof(uint32_t));
...
void TableBuilder_BACS::FinishDataBlock(BlockBuilder* block,
                                        BlockHandle* handle,
...
void TableBuilder_BACS::FinishFilterBlock(FullFilterBlockBuilder* block,
                                          BlockHandle* handle,
                                          CompressionType compressiontype,
                                          size_t& block_size) {
  Rep* r = rep_;
...
void TableBuilder_BACS::FlushData() {
  ...
  if (r->data_inuse_start == -1) {
    ...
    rdma_mg->RDMA_Write(remote_mr, r->local_data_mr[0], msg_size,
                        r->type_string_, IBV_SEND_SIGNALED, 0,
                        rep_->target_node_id_);
    r->data_inuse_end = 0;
    ...
    rdma_mg->RDMA_Write(remote_mr, r->local_data_mr[r->data_inuse_end],
                        msg_size, r->type_string_, IBV_SEND_SIGNALED, 0,
                        rep_->target_node_id_);
...
Status TableBuilder_BACS::Finish() {
  ...
  if (ok()) {
    ...
    //      std::string handle_encoding;
    //      r->pending_data_handle.set_offset(r->offset_last_added);// This is
    //      the offset of the begginning of this block.
...
void TableBuilder_BACS::get_dataindexblocks_map(
...
size_t TableBuilder_BACS::get_numentries() { return rep_->num_entries; }
