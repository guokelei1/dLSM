...
//
// Make sure the Prefetch Granularity is larger than the key value size other
// wise there
// would be error
...
ByteAddressableSEQIterator::ByteAddressableSEQIterator(
  ...
  Env::Default()->rdma_mg->Allocate_Local_RDMA_Slot(*prefetched_mr,
                                                    FlushBuffer);
}
...
void ByteAddressableSEQIterator::Next() { GetNextKV(); }
...
void ByteAddressableSEQIterator::GetNextKV() {
  ...
  if (iter_ptr == nullptr ||
      iter_ptr == remote_mr_current.length + (char*)prefetched_mr->addr) {
    valid_ = Fetch_next_buffer_initial(iter_offset);
    DEBUG_arg("Move to the next chunk, iter_ptr now is %p\n", iter_ptr);
    // TODO: reset all the relevent metadata such as iter_ptr,
    // cur_prefetch_status.
  }
  if (!valid_) {
    return;
  }
  // TODO: The Get KV need to wait if the data has not been fetched already,
  // need to Poll completion
  //  Use the cur_prefetch_status to represent the postion for current
  //  prefetching.
  //    valid_ = true;
  auto rdma_mg = Env::Default()->rdma_mg;
  // Only support forward iterator for sequential access iterator.
...
bool ByteAddressableSEQIterator::Fetch_next_buffer_initial(size_t offset) {
  ...
  prefetch_counter = 0;
  if (Find_prefetch_MR(&tablemeta->remote_data_mrs, offset,
                       &remote_mr_current)) {
    ...
    if (remote_mr_current.length > PREFETCH_GRANULARITY) {
      ...
      //      remote_mr_current.addr = (void*)((char*)remote_mr_current.addr +
      //      PREFETCH_GRANULARITY); remote_mr_current.length -=
      //      PREFETCH_GRANULARITY;
    ...
    //      remote_mr.addr = (void*)((char*)remote_mr.addr +
    //      i*PREFETCH_GRANULARITY); local_mr.addr =
    //      (void*)((char*)local_mr.addr + i*PREFETCH_GRANULARITY); if
    //      (total_len > PREFETCH_GRANULARITY){
    //
    //        rdma_mg->RDMA_Read(&remote_mr, &local_mr, PREFETCH_GRANULARITY,
    //        "read_local", IBV_SEND_SIGNALED, 1); total_len -=
    //        PREFETCH_GRANULARITY;
    //      }else if (total_len > 0){
    //        remote_mr.addr = (void*)((char*)remote_mr.addr +
    //        i*PREFETCH_GRANULARITY); local_mr.addr =
    //        (void*)((char*)local_mr.addr + i*PREFETCH_GRANULARITY);
    //        remote_mr.length = total_len;
    //        local_mr.length = total_len;
    //        rdma_mg->RDMA_Read(&remote_mr, &local_mr, total_len, "read_local",
    //        IBV_SEND_SIGNALED, 0); total_len = 0;
    //      }
    //    }
...
bool ByteAddressableSEQIterator::Fetch_next_buffer_middle() {
  ...
  ibv_mr remote_mr = remote_mr_current;
  remote_mr.addr =
      (char*)remote_mr.addr + PREFETCH_GRANULARITY * prefetch_counter;
  ibv_mr local_mr = *prefetched_mr;
  local_mr.addr =
      (char*)local_mr.addr + PREFETCH_GRANULARITY * prefetch_counter;

  // Note remote_mr_current.length will not be changed in this function, it will
  // only be
  //  changed in the initial function.
  if (remote_mr_current.length - prefetch_counter * PREFETCH_GRANULARITY >
      PREFETCH_GRANULARITY) {
    remote_mr.length = PREFETCH_GRANULARITY;
    local_mr.length = PREFETCH_GRANULARITY;
    rdma_mg->RDMA_Read(&remote_mr, &local_mr, PREFETCH_GRANULARITY,
