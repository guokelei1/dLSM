...
void Find_Remote_MR(std::map<uint32_t, ibv_mr*>* remote_data_blocks,
  ...
  //  assert(position + handle.size() + kBlockTrailerSize <=
  //  iter->second->length); assert(handle.size() +kBlockTrailerSize <= )
  *(remote_mr) = *(iter->second);
  //      DEBUG_arg("Block buffer position %lu\n", position);
  remote_mr->addr =
      static_cast<void*>(static_cast<char*>(iter->second->addr) + position);
}
...
void Find_Local_MR(std::map<uint32_t, ibv_mr*>* remote_data_blocks,
  ...
  //  iter->second->length); assert(handle.size() +kBlockTrailerSize <= )
  //      DEBUG_arg("Block buffer position %lu\n", position);
  data.Reset((static_cast<char*>(iter->second->addr) + position),
             handle.size());
}
...
// TODO: Make the block mr searching and creating outside this function, so that
// datablock is
...
Status ReadDataBlock(std::map<uint32_t, ibv_mr*>* remote_data_blocks,
  ...
  // TODO: Make it use thread local read buffer rather than allocate one every
  // time.
  // #ifdef GETANALYSIS
  ...
  auto duration1 =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop1 - start1);
  ...
  //  const char* data = static_cast<char*>(contents->addr);  // Pointer to
  //  where Read put the data
  ...
  switch (data[n]) {
    ...
    default:
      ...
      assert(false);
      //      rdma_mg->Deallocate_Local_RDMA_Slot(static_cast<void*>(const_cast<char
      //      *>(data)), DataChunk);
  ...
  //   - start); printf("CRC check elapse is %zu\n",  duration.count());
  // #endif
  return Status::OK();
}
...
//  }
  ...
  const char* data =
      static_cast<char*>(contents->addr);  // Pointer to where Read put the data
