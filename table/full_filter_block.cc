...
FullFilterBlockBuilder::FullFilterBlockBuilder(ibv_mr* mr,
    ...
    : local_mr(mr),
      bits_per_key_(bloombits_per_key),
...
//   //result plus filter offsets plus array offset plus 1 char for
//   kFilterBaseLg return (result.size() + filter_offsets_.size()*4 +5);
// }
void FullFilterBlockBuilder::AddKey(const Slice& key) {
#ifndef NDEBUG
  if (key.size() == 29) printf("here\n");
#endif
...
void FullFilterBlockBuilder::Move_buffer(const char* p) { result.Reset(p, 0); }
...
//   rdma_mg_->RDMA_Write(remote_mr, (*local_mrs)[0], msg_size,
//   type_string_,IBV_SEND_SIGNALED, 0); remote_mr->length = msg_size;
...
FullFilterBlockReader::FullFilterBlockReader(
  ...
  num_probes_ = static_cast<int>(contents.data()[len_with_meta - 5]);
...
// bool FullFilterBlockReader::KeyMayMatch(uint64_t block_offset, const Slice&
// key) {
...
FullFilterBlockReader::~FullFilterBlockReader() {
  if (filter_side == Compute) {
    if (!rdma_mg_->Deallocate_Local_RDMA_Slot((void*)filter_content.data(),
                                              FilterChunk)) {
