...
// Note: if the iterator can not seek the same target, it will stop at the key
// right before or
...
//    %lu\n", this, bhandle.offset(), bhandle.size());
    ...
    if (handle.compare(data_block_handle_) == 0) {
      ...
      if (compute_side_) {
        ...
        //  allocate and deallocate again. we abandon the function pointer
        //  design, directly call the static function instead.
        if (mr_addr != nullptr) {
          auto rdma_mg = Env::Default()->rdma_mg;
          rdma_mg->Deallocate_Local_RDMA_Slot(mr_addr, DataChunk);
        }
