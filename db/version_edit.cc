...
namespace dLSM {
// std::shared_ptr<RDMA_Manager> RemoteMemTableMetaData::rdma_mg =
// Env::Default()->rdma_mg; RemoteMemTableMetaData::RemoteMemTableMetaData()  :
// table_type(0), allowed_seeks(1 << 30) {
...
RemoteMemTableMetaData::RemoteMemTableMetaData(int machine_type,
    ...
    : this_machine_type(machine_type),
      shard_target_node_id(id),
      allowed_seeks(1 << 30),
      table_cache(cache) {
  //  assert(shard_target_node_id < 36);
  // Tothink: Is this_machine_type the same as rdma_mg->node_id?
  // Node_id is unique for every node, while the this_machine_type only
  // distinguish the compute node from the memory node.
  if (machine_type == 0) {
    rdma_mg = Env::Default()->rdma_mg;
...
RemoteMemTableMetaData::RemoteMemTableMetaData(int side)
    : this_machine_type(side),
      shard_target_node_id(99),
      allowed_seeks(1 << 30),
      table_cache(nullptr) {
  // Tothink: Is this_machine_type the same as rdma_mg->node_id?
  //  Node_id is unique for every node, while the this_machine_type only
...
RemoteMemTableMetaData::~RemoteMemTableMetaData() {
  // TODO and Tothink: when destroy this metadata check whether this is compute
  // node, if yes, send a message to
  //  home node to deference. Or the remote dereference is conducted in the
  ...
  if (this_machine_type == 0) {
    ...
    if (creator_node_id == rdma_mg->node_id) {
      // #ifndef NDEBUG
      //         printf("Destroying RemoteMemtableMetaData locally on compute
      //         node, Table number is %lu, creator node id is %d \n", number,
      //         creator_node_id);
      // #endif
      ...
      // #ifndef NDEBUG
      //         printf("chunks will be garbage collected on the memory node,
      //         Table number is %lu, "
      //             "creator node id is %d index block pointer is %p\n",
      //             number, creator_node_id,
      //             remote_dataindex_mrs.begin()->second->addr);
      // #endif
      //         assert(remote_dataindex_mrs.size() == 1);
    ...
    for (auto it = remote_dataindex_mrs.begin();
         it != remote_dataindex_mrs.end(); it++) {
      delete it->second;
    }
    for (auto it = remote_filter_mrs.begin(); it != remote_filter_mrs.end();
         it++) {
      delete it->second;
    }
  }
...
void RemoteMemTableMetaData::EncodeTo(std::string* dst) const {
  ...
  dst->append(reinterpret_cast<const char*>(&creator_node_id),
              sizeof(creator_node_id));
  dst->append(reinterpret_cast<const char*>(&shard_target_node_id),
              sizeof(shard_target_node_id));
  ...
  // Here we suppose all the remote memory chuck for the same table have similar
  // infomation  below,
...
Status RemoteMemTableMetaData::DecodeFrom(Slice& src) {
  ...
  for (auto i = 0; i < remote_data_chunk_num; i++) {
    ...
    ibv_mr* mr =
        new ibv_mr{.context = reinterpret_cast<ibv_context*>(context_temp),
  ...
  for (auto i = 0; i < remote_filter_chunk_num; i++) {
    ...
    ibv_mr* mr = new ibv_mr{
      context : reinterpret_cast<ibv_context*>(context_temp),
      pd : reinterpret_cast<ibv_pd*>(pd_temp),
      addr : nullptr,
      length : 0,
      handle : handle_temp,
...
void RemoteMemTableMetaData::mr_serialization(std::string* dst,
                                              ibv_mr* mr) const {
...
void VersionEdit::EncodeTo(std::string* dst) const {
  ...
  for (const auto& deleted_file_kvp : deleted_files_) {
    ...
    dst->append(reinterpret_cast<const char*>(&std::get<2>(deleted_file_kvp)),
                sizeof(uint8_t));
    //    printf("level is %d, number is %lu, node_id is %u",
    //    std::get<0>(deleted_file_kvp),
  ...
  //  assert(dst->size() <
  //  new_files_[0].second->rdma_mg->name_to_size["version_edit"]);
}
void VersionEdit::EncodeToDiskFormat(std::string* dst) const {
  ...
  for (size_t i = 0; i < new_files_.size(); i++) {
    ...
    dst->append(reinterpret_cast<const char*>(&f->creator_node_id),
                sizeof(f->creator_node_id));
    ...
    switch (tag) {
      ...
      case kDeletedFile:
        if (GetLevel(&input, &level) && GetVarint64(&input, &number)) {
          ...
          //          printf("level is %d, number is %lu, node_id is %u", level,
          //          number, node_id);
        } else {
          msg = "deleted file";
        }
      ...
      case kNewFile:
        if (GetLevel(&input, &level)) {
          std::shared_ptr<RemoteMemTableMetaData> f =
              std::make_shared<RemoteMemTableMetaData>(this_machine_type, cache,
                                                       999);
          f->DecodeFrom(input);
          //          assert(level == 0);
...
void VersionEdit_Merger::merge_one_edit(VersionEdit* edit) {
  for (auto iter : *edit->GetDeletedFiles()) {
    ...
    if (new_files_.erase(std::get<1>(iter)) == 0) {
      ...
      // if there is no file already and it is a trival change then this file do
      // not
      //  need to be unpin.
      if (edit->IsTrival()) {
      ...
      if (!edit->IsTrival()) {
        //        assert(std::find(merged_file_numbers.begin(),
        //        merged_file_numbers.end(),
        //                         std::get<1>(iter)) ==
        //                         merged_file_numbers.end());

        // if the merged file is a result from the element in the
        // only_trival_change.
        if (only_trival_change.find(std::get<1>(iter)) !=
            only_trival_change.end()) {
          // if we just delete a sstable which only have trival change according
          // to the merge edit. Then we need to remove it from the only trival
          // change list. if we do not do this, the fucntion PersistSSTables
          // will create more threads than expected. That may cause system
          // crash.
      ...
      //  do not unpin here, unpin during the consistent check point. we do
      //  nothing here
    }
  }
