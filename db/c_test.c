...
static void CheckGet(dLSM_t* db, const dLSM_readoptions_t* options,
...
static void CheckIter(dLSM_iterator_t* iter, const char* key, const char* val) {
  size_t len;
  const char* str;
...
int main(int argc, char** argv) {
  ...
  for (run = 0; run < 2; run++) {
    ...
    if (run == 0) {
      policy = dLSM_filterpolicy_create(NULL, FilterDestroy, FilterCreate,
