...
#define MEMTABLE_SEQ_SIZE \
  153846  // Make the in memory buffer close to 64MB 1 shard origin
// #define MEMTABLE_SEQ_SIZE 19231 //Make the in memory buffer close to 64MB
// #define MEMTABLE_SEQ_SIZE 38462
//  8 shard's memtable size X 8  should be a little larger that the memtable of
//  64MB, because of the merge #define MEMTABLE_SEQ_SIZE 610081
...
class MemTable {
 public:
  ...
  // Requested means in the queue but not handled by thread, scheduled means put
  // into the
  enum FlushStateEnum {
    FLUSH_NOT_REQUESTED,
    FLUSH_REQUESTED,
  ...
  void SetLargestSeq(uint64_t seq) { largest_seq_supposed = seq; }
  void SetFirstSeq(uint64_t seq) { first_seq = seq; }
  //  void SetLargestSeqTillNow(uint64_t seq){
  //
  //  }
  bool CheckFlushInProcess() { return flush_state_ == FLUSH_PROCESSING; }
  bool CheckFlush_Requested() { return flush_state_ == FLUSH_REQUESTED; }
  bool CheckFlushFinished() { return flush_state_.load() == FLUSH_FINISHED; }
  void SetFlushState(FlushStateEnum state) { flush_state_.store(state); }
  void MarkFlushed() {
    assert(flush_state_ == FLUSH_PROCESSING);
    SetFlushState(FLUSH_FINISHED);
  }
  uint64_t Getlargest_seq_supposed() const {
    // in case that there is a unfull table flush, the largest seq will be
    // different from the one supposed
  ...
  uint64_t GetFirstseq() const { return first_seq; }
  void increase_seq_count(size_t num) {
    seq_count.fetch_add(num);
    assert(num == 1);
    assert(seq_count <= MEMTABLE_SEQ_SIZE);
    // TODO; For a write batch you the write may cross the boder, we need to
    // modify
  ...
  size_t Get_seq_count() { return seq_count; }
