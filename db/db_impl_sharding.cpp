...
DBImpl_Sharding::DBImpl_Sharding(const Options& options,
  ...
  for (const auto& iter : Shard_Info) {
    // We can not set the target node id in DBImpl because we don't know what
    // should be
    //  the node id corresponding with this shard. (Is that true?) Probably not.

    // Now the shards are assigned to target memory nodes in a strictly round
    // robin manner according to the upper bound of shard. the third argument we
    // set as 0, to overload the function. The overloaded initial function will
    // not create message handling thread.
    auto sharded_db = new DBImpl(options, dbname, iter.second, iter.first);
  ...
  // you should not combine this two loops together because there is a wait
  // function.
  //    for(auto & iter : shards_pool){
