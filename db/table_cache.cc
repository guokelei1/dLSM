...
TableCache::~TableCache() {
  ...
  if (TableCache::GetNum.load() > 0)
    printf(
        "Cache Get time statics is %zu, %zu, %zu, need binary search: "
  ...
  if (TableCache::not_filtered.load() > 0) {
    printf(
        "Average time elapse for Data binary search is %zu, "
        "Average time elapse for Index binary search is %zu,"
        " Average time elapse for data block fetch before table_cache is %zu\n",
        TableCache::DataBinarySearchTimeElapseSum.load() /
            TableCache::not_filtered.load(),
        TableCache::IndexBinarySearchTimeElapseSum.load() /
            TableCache::not_filtered.load(),
        TableCache::DataBlockFetchBeforeCacheElapseSum.load() /
            TableCache::not_filtered.load());
  }
  if (TableCache::cache_miss > 0 && TableCache::cache_hit > 0) {
    printf(
        "Cache hit Num %zu, average look up time %zu, Cache miss %zu, average "
        "Block Fetch time %zu\n",
        TableCache::cache_hit.load(),
        TableCache::cache_hit_look_up_time.load() /
            TableCache::cache_hit.load(),
        TableCache::cache_miss.load(),
        TableCache::cache_miss_block_fetch_time.load() /
            TableCache::cache_miss.load());
  }
#endif
  printf("Total number of entries within the cahce is %zu",
         cache_->TotalCharge());
  delete cache_;
}
...
Status TableCache::FindTable(
  ...
  //  sizeof(Remote_memtable_meta->creator_node_id)]; EncodeFixed64(buf,
  //  Remote_memtable_meta->number); memcpy(buf +
  //  sizeof(Remote_memtable_meta->number),
  //  &Remote_memtable_meta->creator_node_id,
  ...
  *handle = cache_->Lookup(key);
  if (*handle == nullptr) {
    // TODO: implement a hash lock to reduce the contention here, otherwise
    // multiple readers may get the same table and RDMA read the index block
    // several times.
    ...
    *handle = cache_->Lookup(key);
    if (*handle == nullptr) {
      Table* table = nullptr;
      //          printf("Did not find the table in the table_cache, file number
      //          is %lu \n ", Remote_memtable_meta->number);
      if (s.ok()) {
        s = Table::Open(options_, &table, Remote_memtable_meta);
      }
      // TODO(ruihong): add remotememtablemeta and Table to the table_cache
      // entry.
      if (!s.ok()) {
        assert(table == nullptr);
        //      delete file;
        // We do not table_cache error results so that if the error is
        // transient, or somebody repairs the file, we recover automatically.
      } else {
        assert(table != nullptr);
...
Status TableCache::FindTable_MemorySide(
    ...
    Table_Memory_Side*& table) {
  {
    ...
    //      DEBUG_arg("file number inserted to the table_cache is %lu ",
    //      Remote_memtable_meta.get()->number); DEBUG_arg("Remote_memtable_meta
    //      pointer is %p\n", Remote_memtable_meta.get());
    return s;
  }
  // Status
  // TableCache::FindTable_MemorySide(std::shared_ptr<RemoteMemTableMetaData>
  // Remote_memtable_meta, Cache::Handle** handle){
  //   {
  //     Status s;
  //     char buf[sizeof(Remote_memtable_meta->number) +
  //     sizeof(Remote_memtable_meta->creator_node_id)]; EncodeFixed64(buf,
  ...
  //       if (s.ok()) {
  //         s = Table_Memory_Side::Open(options_, &table,
  //         Remote_memtable_meta);
  //         //      DEBUG_arg("file number inserted to the table_cache is %lu
  //         ", Remote_memtable_meta.get()->number);
  ...
  //         assert(table->rep != nullptr);
  //         //
  //         assert(static_cast<RemoteMemTableMetaData*>(table->Get_remote_table_ptr())->number
  //         != 0); *handle = table_cache->Insert(key, tf, 1,
  //         &DeleteEntry_Memory);
  //       }
  //     }
  //     return s;
...
Iterator* TableCache::NewIterator(
  ...
  Table* table =
      reinterpret_cast<SSTable*>(cache_->Value(handle))->table_compute;
...
Iterator* TableCache::NewIterator_MemorySide(
  ...
  // #endif
  //  TODO: Shall we garbage collect the Table_Memory_Side since we will not put
  //  it into cache?
  Table_Memory_Side* table;
  ...
  //  Table_Memory_Side* table =
  //  reinterpret_cast<SSTable*>(table_cache->Value(handle))->table_memory;
  // The pointer of p will be different from what stored in the table_cache
  // because the p is a new created Remote memory metadata from the serialized
  // buffer. so no need for the assert of p == table->Get_rdma()
  assert(table->Get_remote_table_ptr() != nullptr);
  Iterator* result = table->NewIterator(options);
...
#endif
  Cache::Handle* handle = nullptr;
  // TODO: not let concurrent thread finding the same tale and inserting the
  // same
