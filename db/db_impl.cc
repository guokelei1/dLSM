...
#endif
{
  ...
  //         new VersionSet(dbname_, &options_, table_cache_,
  //         &internal_comparator_,
  //                        &superversion_memlist_mtx, iter.first,
  //                        iter.second)})
  //  }
  ...
  env_->SetBackgroundThreads(options_.max_background_flushes,
                             ThreadPoolType::FlushThreadPool);
  env_->SetBackgroundThreads(options_.max_background_compactions,
                             ThreadPoolType::CompactionThreadPool);
  // TODO: Make client handling thread only 1 per compute node-memory node
  // connection.
  ...
  //    for (int i = 0; i < rdma_mg->memory_nodes.size(); ++i) {
  //      main_comm_threads.emplace_back(
  //          &DBImpl::client_message_polling_and_handling_thread, this,
  //          "main");
  //      main_comm_threads.back().detach();
  //    }

  printf("communication thread created\n");
  // Wait for the clearance of pending receive work request from the last DB
  // open.
  //    {
  ...
  //      size_t size_in_gb =
  //      options_.block_cache->GetCapacity()/(1024*1024*1024) +1 +1; for
  //      (size_t i = 0; i < size_in_gb; ++i) {
  //        ibv_mr* mr;
  //        char* buff;
  //        rdma_mg->Local_Memory_Register(&buff, &mr, 1024*1024*1024,
  //        DataChunk);
  //      }
  //    }
  // Preallocate the RDMA local buffer. ADD extra 1GB to make sure covering all
  // the table_cache.
  ...
  //          assert(rdma_mg->res->qp_main_connection_info.find(trial)!=
  //          rdma_mg->res->qp_main_connection_info.end()); l.unlock();
...
// This functon does not contain the creation of the client message handling
// thread
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname,
...
//  }
  ...
  if (RDMA_Manager::ReadCount.load() != 0)
    printf(
        "RDMA read operatoion average time duration: %zu, ReadNuM is%zu, "
        "total time is %zu\n",
        RDMA_Manager::RDMAReadTimeElapseSum.load() /
  ...
  if (RDMA_Manager::ReadCount1.load() != 0)
    printf(
        "****Find MR average time duration: %zu, Allocate MR is %zu, "
        "Read NUm is %zu ****\n",
        RDMA_Manager::RDMAFindmrElapseSum.load() /
            RDMA_Manager::ReadCount1.load(),
        RDMA_Manager::RDMAMemoryAllocElapseSum.load() /
            RDMA_Manager::ReadCount1.load(),
        RDMA_Manager::ReadCount1.load());
#endif
  ...
  if (flush_times.load() > 0)
    printf(
        "Memtable total flush time, number of flush, average flush time are "
        "%zu, %zu, %zu\n",
        Total_time_elapse.load(), flush_times.load(),
        Total_time_elapse.load() / flush_times.load());
#endif
}
// put the memtable to immutable table and flush all immutable to remote memory
// if the immutable trigger is 1. Wait for all the background task to finish.
void DBImpl::WaitforAllbgtasks(bool clear_mem) {
  if (clear_mem) {
    std::unique_lock<std::mutex> lck(superversion_memlist_mtx);
    MemTable* mem = mem_.load();
    // Use an iterator to check whether the current memtable is empty, if it is
    // not
    ...
    if (iter->Valid()) {
      ...
      DEBUG_arg("Not full flushed table first seq number is %lu",
                mem->GetFirstseq());
      ...
      // starting from this sequenctial number, the data should write to the new
      // memtable set the immutable as seq_num - 1
      ...
      assert(imm_.current_memtable_num() <=
             config::Immutable_StopWritesTrigger);
      imm_.Add(mem);
    ...
    if (ParseFileName(filename, &number, &type)) {
      ...
      switch (type) {
        ...
        case kDescriptorFile:
          ...
          //          "live" keep = (live.find(number) != live.end()); break;
        case kCurrentFile:
        case kDBLockFile:
        case kInfoLogFile:
          keep = true;
...
Status DBImpl::WriteLevel0Table(FlushJob* job, VersionEdit* edit) {
  ...
  std::shared_ptr<RemoteMemTableMetaData> meta =
      std::make_shared<RemoteMemTableMetaData>(0, versions_->table_cache_,
  ...
  if (s.ok() && meta->file_size > 0) {
    ...
    //    }
    meta->level = 0;
    // No need to add file here, we will add to the version edit in Try install
    // flush
    //  result
    //    edit->AddFile(0, meta);
...
//  size_t counter = 0;
//  // wait for the immutable to get ready to flush. the signal here is prepare
//  for
//  // the case that the thread this immutable is under the control of
//  conditional
//  // variable.
////-----------------seldom Lock----------------------------
...
//////    printf("mem %p has is still alive\n", mem);
//////    printf("After mem dereference head node of the imm %p\n",
/// mem->GetTable()->GetHeadNode()->Next(0)); /    imm_.store(nullptr);
//////    undefine_mutex.Unlock();
//////    write_stall_cv.SignalAll();
////    has_imm_.store(false, std::memory_order_release);
////    // TODO how to remove the obsoleted remote memtable?
...
void DBImpl::CompactMemTable() {
  ...
  size_t counter = 0;
  // wait for the immutable to get ready to flush. the signal here is prepare
  // for the case that the thread this immutable is under the control of
  // conditional variable.
  FlushJob f_job(&write_stall_cv, &internal_comparator_);
  {  // This code should synchronized outside the superversion mutex, since
     // nothing
...
void DBImpl::CompactRange(const Slice* begin, const Slice* end) {
  ...
  //  TEST_CompactMemTable();  // TODO(sanjay): Skip if memtable does not
  //  overlap
...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  //  undefine_mutex.AssertHeld();
  // In my implementation the Maybeschedule Compaction will only be triggered
  // once by the thread which CAS the memtable successfully.
  ...
  if (imm_.IsFlushPending()) {
    ...
    BGThreadMetadata* thread_pool_args =
        new BGThreadMetadata{.db = this, .func_args = function_args};
  ...
  if (versions_->NeedsCompaction()) {
    ...
    BGThreadMetadata* thread_pool_args =
        new BGThreadMetadata{.db = this, .func_args = function_args};
    env_->Schedule(BGWork_Compaction, static_cast<void*>(thread_pool_args),
                   ThreadPoolType::CompactionThreadPool);
...
void DBImpl::BackgroundCompaction(void* p) {
  ...
  if (shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (c == nullptr) {
      ...
      auto start = std::chrono::high_resolution_clock::now();
      //      write_stall_mutex_.AssertNotHeld();
      // Only when there is enough input level files and output level files will
      // the subcompaction triggered
      if (options_.usesubcompaction && c->num_input_files(0) >= 4 &&
          c->num_input_files(1) > 1) {
      ...
      auto duration =
          std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
      printf(
          "Table compaction time elapse (%ld) us, compaction level is %d, "
          "first level file number %d, the second level file number %d \n",
          duration.count(), compact->compaction->level(),
          compact->compaction->num_input_files(0),
          compact->compaction->num_input_files(1));
      DEBUG("Non-trivalcompaction!\n");
      std::cout << "compaction task table number in the first level"
                << compact->compaction->inputs_[0].size() << std::endl;
      if (!status.ok()) {
        RecordBackgroundError(status);
      }
  ...
  if (shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (c == nullptr) {
      ...
      c->edit()->AddFile(c->level() + 1, f);
      {
        ...
        //        std::unique_lock<std::mutex> l_vs(versionset_mtx,
        //        std::defer_lock);
        f->level = c->level() + 1;
        status = versions_->LogAndApply(c->edit());
...
Status DBImpl::OpenCompactionOutputFile(SubcompactionState* compact) {
  ...
  if (s.ok()) {
    ...
    compact->builder =
        new TableBuilder_ComputeSide(options_, Compact, shard_target_node_id);
#endif
#ifdef BYTEADDRESSABLE
    compact->builder =
        new TableBuilder_BACS(options_, Compact, shard_target_node_id);
#endif
  }
  return s;
}
...
Status DBImpl::FinishCompactionOutputFile(SubcompactionState* compact,
  ...
  compact->builder->get_datablocks_map(
      compact->current_output()->remote_data_mrs);
  compact->builder->get_dataindexblocks_map(
      compact->current_output()->remote_dataindex_mrs);
  compact->builder->get_filter_map(
      compact->current_output()->remote_filter_mrs);
#ifndef NDEBUG
...
//  imm_mtx->lock();
#ifndef NDEBUG
  if (mems.size() == 1) DEBUG("check 1 memtable installation\n");
#endif
  ...
  // First apply locally then apply remotely.
  {
    ...
    //     lck.unlock();
    // #endif
    // In stallSuperVersion should be outside the version set lock, other wise
    // there
    //  will be a deadlock
    InstallSuperVersion();
  }
...
SuperVersion* DBImpl::GetThreadLocalSuperVersion() {
  ...
  if (sv == SuperVersion::kSVObsolete ||
    ...
    // if there is an old superversion unrefer old one and replace it as a new
    // one
...
void DBImpl::InstallSuperVersion() {
  ...
  // need to be protected by a versionset mutex. or make the refs an atomic
  // value.
...
printf("null current\n");
  ...
  //  *(uint32_t*)((char*)send_mr_c.addr + serilized_c.size() +
  //  sizeof(receive_mr.addr))\
//      = recv_mr_c.rkey;
...
//             sizeof(receive_mr.addr) + sizeof(receive_mr.rkey), 1, 1);
  ...
  // Without persistency we don' need to reply to the remote memory after the
  // compute node got the version edit.
#ifdef WITHPERSISTENCE
  ...
  *receive_pointer = {};
  ...
  asm volatile("mfence\n" : :);
#endif
  rdma_mg->post_send<RDMA_Request>(&send_mr, shard_target_node_id,
                                   std::string("main"));
  ibv_wc wc[2] = {};
  ...
  asm volatile("lfence\n" : :);
  asm volatile("mfence\n" : :);
  if (!rdma_mg->poll_reply_buffer(
          receive_pointer))  // poll the receive for 2 entires
  {
  ...
  // TODO: Clear the large receive buffer before it receive the buffer size,
  // this can be optimized
  //  to only clear the corresponding byte. Besides, if the compaction finished
  //  extremely fast,
  ...
  // RDMA write back was depracated. use an RDMA read from the remote memory
  // could be
  //  a better choice.
  //  asm volatile ("sfence\n" : : );
  //  asm volatile ("lfence\n" : : );
  //  asm volatile ("mfence\n" : : );
  //  //Note: here multiple threads will RDMA_Write the "main" qp at the same
  //  time,
  //  // which means the polling result may not belongs to this thread, but it
  //  does not
  //  // matter in our case because we do not care when will the message arrive
  //  at the other side rdma_mg->RDMA_Write(remote_large_prt, remote_large_rkey,
  ...
  //
  //    //TODO: implement compaction pool waiting here, wait up it from the RPC
  //    handling thread
  ...
  // THe polling byte is determined after we receive the buffer size, so we
  // set all the buffer bytes as '\0'
  //  volatile unsigned char* polling_size_2 = (unsigned char*)recv_mr_c.addr +
  //  buffer_size - 1; *polling_size_2 = 0;
  asm volatile("sfence\n" : :);
  asm volatile("lfence\n" : :);
  asm volatile("mfence\n" : :);
  ...
  *file_number_end_send_ptr = file_number_end;
  ...
  rdma_mg->RDMA_Write(remote_prt, remote_rkey, &send_mr, sizeof(uint64_t) + 1,
                      "main", IBV_SEND_SIGNALED, 1);
...
void DBImpl::sync_option_to_remote(uint8_t target_node_id) {
  ...
  *receive_pointer = {};
  ...
  asm volatile("mfence\n" : :);
  //  sleep(1);
  rdma_mg->post_send<RDMA_Request>(&send_mr, target_node_id,
                                   std::string("main"));
  ...
  //  which means the polling result may not belongs to this thread, but it does
  //  not matter in our case because we do not care when will the message arrive
  //  at the other side.
  asm volatile("sfence\n" : :);
  asm volatile("lfence\n" : :);
  asm volatile("mfence\n" : :);
...
void DBImpl::remote_qp_reset(std::string& qp_type, uint8_t target_node_id) {
  ...
  rdma_mg->sock_sync_data(rdma_mg->res->sock_map[target_node_id], 1, temp_send,
                          temp_receive);
...
void DBImpl::client_message_polling_and_handling_thread(std::string q_id) {
  ...
  if (q_id == "read_local") {
    ...
    qp = static_cast<ibv_qp*>(
        rdma_mg->qp_local_read.at(shard_target_node_id)->Get());
    if (qp == NULL) {
      ...
      // if the qp has already been existed re initialize the qp to clear the
      // old WRQs.
      assert(rdma_mg->local_read_qp_info.at(shard_target_node_id)->Get() !=
             nullptr);
      //        rdma_mg->modify_qp_to_reset(qp);
    ...
    qp = static_cast<ibv_qp*>(
        rdma_mg->qp_local_write_flush.at(shard_target_node_id)->Get());
    if (qp == NULL) {
      rdma_mg->Remote_Query_Pair_Connection(q_id, shard_target_node_id);
      qp = static_cast<ibv_qp*>(
          rdma_mg->qp_local_write_flush.at(shard_target_node_id)->Get());
      ...
      assert(
          rdma_mg->local_write_flush_qp_info.at(shard_target_node_id)->Get() !=
          nullptr);
      //        rdma_mg->modify_qp_to_reset(qp);
      //        rdma_mg->connect_qp(qp, q_id);
    }
    ...
    qp = static_cast<ibv_qp*>(
        rdma_mg->qp_local_write_compact.at(shard_target_node_id)->Get());
    if (qp == NULL) {
      rdma_mg->Remote_Query_Pair_Connection(q_id, shard_target_node_id);
      qp = static_cast<ibv_qp*>(
          rdma_mg->qp_local_write_compact.at(shard_target_node_id)->Get());
    } else {
      assert(rdma_mg->local_write_compact_qp_info.at(shard_target_node_id)
                 ->Get() != nullptr);
      //        rdma_mg->modify_qp_to_reset(qp);
      //        rdma_mg->connect_qp(qp, q_id);
    }
    ...
    if (rdma_mg->res->qp_map.find(shard_target_node_id) !=
        rdma_mg->res->qp_map.end()) {
      //        qp = rdma_mg->res->qp_map.at(q_id);
      //        printf("qp number before reset is %d\n", qp->qp_num);
      //        assert(rdma_mg->res->qp_main_connection_info.find(q_id)!=
      //        rdma_mg->res->qp_main_connection_info.end()); l.unlock();
  ...
  if (rdma_mg->comm_thread_recv_mrs.find(shard_target_node_id) !=
      rdma_mg->comm_thread_recv_mrs.end()) {
    ...
    for (int i = 0; i < R_SIZE; i++) {
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[i], shard_target_node_id,
                                          q_id);
    }
    buffer_counter = 0;
  ...
  while (!shutting_down_.load()) {
    ...
    if (rdma_mg->try_poll_completions(wc, 1, q_id, false,
      ...
      memcpy(receive_msg_buf, recv_mr[buffer_counter].addr,
             sizeof(RDMA_Request));
...
void DBImpl::Edit_sync_to_remote(VersionEdit* edit,
  ...
  //  - start); printf("Sync version edit time elapse part 1 allocation: %ld us
  //  \n", duration.count()); start = std::chrono::high_resolution_clock::now();
  ...
  //  start); printf("Sync version edit time elapse: %ld us part 2
  ...
  *receive_pointer = {};
  ...
  //  end = std::chrono::high_resolution_clock::now();
  //  duration = std::chrono::duration_cast<std::chrono::microseconds>(end -
  //  start); printf("Sync version edit time elapse: %ld us part 3 memcopy and
  //  WR post\n", duration.count()); start =
  //  std::chrono::high_resolution_clock::now();

  // TODO: we need to lock the whole version edit process. The memory server
  // need to have
  //  a reply to let the compute node release the lock to make sure that the
  //  compute and memory node has the same sequence of version edit. The
  //  difference sequence of version edit may trigger big problem (system
  //  crash), if the new version edit delete a file which has not been installed
  //  by the old edit yet, because the old version edit is lagged because of
  //  RDMA or cpu schedule. For now it is ok.
  //  version_mtx->unlock();
  ...
  //  end = std::chrono::high_resolution_clock::now();
  //  duration = std::chrono::duration_cast<std::chrono::microseconds>(end -
  //  start); printf("Sync version edit time elapse: %ld us part 4 mutex
  //  unlock\n", duration.count()); start =
  //  std::chrono::high_resolution_clock::now();
  asm volatile("sfence\n" : :);
  asm volatile("lfence\n" : :);
  ...
  //  end = std::chrono::high_resolution_clock::now();
  //  duration = std::chrono::duration_cast<std::chrono::microseconds>(end -
  //  start); printf("Sync version edit time elapse: %ld us part 5 receive the
  //  reply\n", duration.count());
  ...
  // TODO: implement a wait function for the received bit. THe problem is when
  // to
  //  reset the buffer to zero (we may need different buffer for the compaction
  //  reply) and how to make sure that the reply will wake up this waiting
  //  thread. The signal may comes befoer the thread go to sleep.
  //
  ...
  //  end = std::chrono::high_resolution_clock::now();
  //  duration = std::chrono::duration_cast<std::chrono::microseconds>(end -
  //  start); printf("Sync version edit time elapse: %ld us part 6 Deallocation
  //  \n", duration.count());
}
void DBImpl::install_version_edit_handler(RDMA_Request* request,
  ...
  if (request->content.ive.trival) {
    ...
    auto f = versions_->current()->FindFileByNumber(
        request->content.ive.level, request->content.ive.file_number,
...
#ifndef NDEBUG
    printf(
        "Get the printed result after %zu iteration, received %d, checkbyte is "
        "%d\n",
...
void DBImpl::persistence_unpin_handler(void* arg) {
  ...
  //  version edit in the first REQUEST from compute node.
  volatile char* polling_byte =
      (char*)file_number_recv_mr.addr + request->content.psu.buffer_size - 1;
  memset((void*)polling_byte, 0, 1);
  for (int i = 0; i < 100; ++i) {
    if (*polling_byte != 0) {
      printf("polling byte error");
...
//   super_version.store(new SuperVersion(mem_,imm_.current(),
//   versions_->current())); super_version.load()->Ref();
// #ifndef NDEBUG
...
Status DBImpl::DoCompactionWorkWithSubcompaction(CompactionState* compact) {
  ...
  //  int subcompaction_num = std::min((int)c->GetBoundariesNum(),
  //  config::MaxSubcompaction);
  if (boundaries->size() <= options_.MaxSubcompaction) {
    ...
    for (int i = 0; i < sizes->size(); i++) {
      if ((*sizes)[i] <= options_.max_file_size / 4) small_files.push_back(i);
    }
    int big_files_num = boundaries->size() - small_files.size();
    int files_per_subcompaction =
        big_files_num / options_.MaxSubcompaction +
        1;  // Due to interger round down, we need add 1.
    double mean = sum * 1.0 / options_.MaxSubcompaction;
    for (size_t i = 0; i <= boundaries->size(); i++) {
      size_t range_size = (*sizes)[i];
      Slice* start = i == 0 ? nullptr : &(*boundaries)[i - 1];
      int files_counter =
          range_size <= options_.max_file_size / 4 ? 0 : 1;  // count this file.
      // TODO(Ruihong) make a better strategy to group the boundaries.
      // Version 1
      //      while (i!=boundaries->size() && range_size < mean &&
      //             range_size + (*sizes)[i+1] <= mean +
      //             3*options_.max_file_size/4){
      //        i++;
      //        range_size += (*sizes)[i];
      //      }
      // Version 2
      while (i != boundaries->size() &&
             (files_counter < files_per_subcompaction ||
              (*sizes)[i + 1] <= options_.max_file_size / 4)) {
        i++;
        size_t this_file_size = (*sizes)[i];
        range_size += this_file_size;
        // Only increase the file counter when add big file.
        if (this_file_size >= options_.max_file_size / 4) files_counter++;
      }
      Slice* end = i == boundaries->size() ? nullptr : &(*boundaries)[i];
...
void DBImpl::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
...
#endif
  Status status;
  // TODO: try to create two ikey for parsed key, they can in turn represent the
  // current user key
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    //    assert(key.data()[0] == '0');
    // We do not need to check whether the output file have too much overlap
    // with level n + 2.
    ...
    // TODO: record the largest key as the last ikey, find a more efficient way
    // to record
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      //        //  Generally delete can only be deleted when there is
      //        definitely no file contain the
    ...
    //    assert(key.data()[0] == '0');
    input->Next();
    // NOTE(ruihong): When the level iterator is invalid it will be deleted and
    // then the key will
    //  be invalid also.
    //    assert(key.data()[0] == '0');
  }
...
//  assert(input->Valid());
  ...
  if (status.ok() && sub_compact->builder != nullptr) {
    ...
    sub_compact->current_output()->largest.DecodeFrom(
        key);  // The SSTable for subcompaction range will be (start, end]
    status = FinishCompactionOutputFile(sub_compact, input);
  }
  if (status.ok()) {
    status = input->status();
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      // TODO: Can we replace this with if (last_sequence_for_key !=
      // kMaxSequenceNumber)
...
struct IterState {
  ...
  IterState(port::Mutex* mutex, MemTable* mem, MemTableListVersion* imm_version,
            Version* version)
...
Iterator* DBImpl::NewInternalIterator(const ReadOptions& options,
  ...
  // TODO: make the user defined snapshot work. THe superversion should be
  // confirmed when creating the snapshot.
  SuperVersion* sv;
  ...
  // for a long time. while we have to return the superversion by the end of
  // this function. Otherwise we can not get the superversion out side this
  // funciton. THe superversion can not exist in such a long existing class. THe
  // fundamental reason is that the superverison can not be gotten twice without
  // return.
  mem->Ref();
  imm->Ref();
  current->Ref(7);
  //  if (sv != nullptr){
  //    sv->Ref();
  //    if (sv == super_version.load()){// there is no superversion change
  //    between.
  //      MemTable* mem = sv->mem;
  //      MemTableListVersion* imm = sv->imm;
  //      Version* current = sv->current;
...
const Snapshot* DBImpl::GetSnapshot() {
  // TODO: get snapshot need to get the superversion before the sequential
  // number
  MutexLock l(&undefine_mutex);
...
//  if (status.ok() && updates != nullptr) {  // nullptr batch is for
//  compactions
//    // TODO: Remove all the Lock, use fettch and add to atomically increase
//    the
...
// and it is supposed to write to the new memtable which has not been created
// yet. hint how about set the metable barrier as seq_num rather than memory
// size?
#ifdef TIMEPRINT
  start = std::chrono::high_resolution_clock::now();
#endif
  if (status.ok()) {  // nullptr batch is for compactions
    ...
    //    }else{
    //      // TODO: some write may have write to immtable, the immutable need
    //      to be notified
    //      // when all the outgoing write on this table have finished and flush
    //      to storage status = WriteBatchInternal::InsertInto(updates, imm_);
    //    }
    assert(sequence <= mem->Getlargest_seq_supposed() &&
           sequence >= mem->GetFirstseq());
...
//  }
  ...
  auto total_duration =
      std::chrono::duration_cast<std::chrono::nanoseconds>(stop - total_start);
...
// seldom Lock
//// TOTHINK The write batch should not too large. other wise the wait function
/// may / memtable could overflow even before the actual write.
Status DBImpl::PickupTableToWrite(bool force, uint64_t seq_num,
                                  MemTable*& mem_r) {
  Status s = Status::OK();
  // Get a snapshot it is vital for the CAS but not vital for the wait logic.
  mem_r = mem_.load();
  // First check whether we need to switch the table, we do not Lock here,
  // because most of the time the memtable will not be switched. we will Lock
  // inside and get the table
  bool delayed = false;
  ...
  while (seq_num > mem_r->Getlargest_seq_supposed()) {
    ...
    if (imm_.current_memtable_num() >= config::Immutable_StopWritesTrigger ||
      ...
      // We check the imm again in the while loop, because the state may have
      // already been changed before you acquire the Lock.
      std::unique_lock<std::mutex> lck(superversion_memlist_mtx);
      //      imm_mtx.lock();
      Log(options_.info_log, "Current memtable full; waiting...\n");
      mem_r = mem_.load();
      while (
          (imm_.current_memtable_num() >= config::Immutable_StopWritesTrigger ||
        ...
        //        std::cout << "Writer is going to wait current immutable number
        //        " << (imm_.current_memtable_num()) << " Level 0 file number "
...
//      assert(locked == false);
      ...
      if (imm_.current_memtable_num() <= config::Immutable_StopWritesTrigger &&
        ...
        temp_mem->SetFirstSeq(last_mem_seq + 1);
        // starting from this sequenctial number, the data should write the the
        // new memtable set the immutable as seq_num - 1
        temp_mem->SetLargestSeq(last_mem_seq + MEMTABLE_SEQ_SIZE);
        temp_mem->Ref();
  ...
  while (true) {
    if (seq_num >= mem_r->GetFirstseq() &&
        seq_num <= mem_r->Getlargest_seq_supposed()) {
      return s;
    } else {
      // get the snapshot for imm then check it so that this memtable pointer is
      // guarantee to be the one this thread want.
      // TODO: use imm_mtx to control the access.
      std::unique_lock<std::mutex> l(superversion_memlist_mtx);
      mem_r = imm_.PickMemtablesSeqBelong(seq_num);
      if (mem_r != nullptr) return s;
    }
  }
}
// TOTHINK The write batch should not too large. other wise the wait function
// may memtable could overflow even before the actual write.
// ---------------Lock free----------------
// Status DBImpl::PickupTableToWrite(bool force, uint64_t seq_num, MemTable*&
// mem_r) {
//  Status s = Status::OK();
//  //Get a snapshot it is vital for the CAS but not vital for the wait logic.
//  mem_r = mem_.load();
...
////      Log(options_.info_log, "Current memtable full; waiting...\n");
////      mem_r = mem_.load();
////      while (imm_.load() != nullptr && seq_num >
/// mem_r->Getlargest_seq_supposed()) {
//        assert(seq_num > mem_r->GetFirstseq());
////        usleep(1);
//        counter++;
//
...
//        sleep_counter++;
//        env_->SleepForMicroseconds(10);// slightly larger than wait for
//        flushing.
////          usleep(10);
//        counter = 0;
//      }
...
//      mem_r->SetFlushState(MemTable::FLUSH_REQUESTED);
//
//      // CAS strong means that one thread will definitedly win under the
//      concurrent,
//      // if it is weak then after the metable is full all the threads may
//      gothrough the while
//      // loop multiple times.
...
//  //if not which table should this writer need to write?
//  while(true){
//    if (seq_num >= mem_r->GetFirstseq() && seq_num <=
//    mem_r->Getlargest_seq_supposed()){
//      return s;
//    }else {
//      // get the snapshot for imm then check it so that this memtable pointer
//      is guarantee
...
// Status DBImpl::MakeRoomForWrite(bool force, uint64_t seq_num, MemTable*&
// mem_r) {
////  undefine_mutex.AssertHeld();
...
///{ /      // There are too many level-0 files. /      Log(options_.info_log,
  ...
  *dbptr = nullptr;
  ...
  if (options.ShardInfo == nullptr) {
    ...
    if (s.ok() && save_manifest) {
      ...
      //    lck(impl->versionset_mtx,std::defer_lock);
    ...
    *dbptr = impl_with_shards;
    ...
    for (auto iter : *impl_with_shards->GetShards_pool()) {
      ...
      // TODO: the target node id should be set before we setup the client
      // handling threads.
