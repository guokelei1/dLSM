...
dLSM::Memory_Node_Keeper::Memory_Node_Keeper(bool use_sub_compaction,
  ...
  struct dLSM::config_t config = {NULL,     /* dev_name */
                                  NULL,     /* server_name */
                                  tcp_port, /* tcp_port */
                                  1,        /* ib_port */
                                  1,        /* gid_idx */
  ...
  // TODO: add a handle function for the option value to get the non-default
  // bloombits.
  opts->filter_policy =
      new InternalFilterPolicy(NewBloomFilterPolicy(opts->bloom_bits));
...
//  void dLSM::Memory_Node_Keeper::Schedule(void
//  (*background_work_function)(void*),
//                                             void* background_work_arg,
//                                             ThreadPoolType type) {
//    message_handler_pool_.Schedule(background_work_function,
//    background_work_arg);
//  }
...
//      BGThreadMetadata* thread_pool_args = new BGThreadMetadata{.db = this,
//      .func_args = function_args}; if (Compactor_pool_.queue_len_.load()>256){
//        //If there has already be enough compaction scheduled, then drop this
//        one printf("queue length has been too long %d elements in the
//        queue\n", Compactor_pool_.queue_len_.load()); return;
//      }
//      Compactor_pool_.Schedule(BGWork_Compaction,
//      static_cast<void*>(thread_pool_args)); printf("Schedule a Compaction
//      !\n");
//    }
//  }
...
////          m->level, (m->begin ? m->begin->DebugString().c_str() :
///"(begin)"), /          (m->end ? m->end->DebugString().c_str() : "(end)"), /
///(m->done ? "(end)" : manual_end.DebugString().c_str())); /    } else {
...
//      f->level = f->level +1;
//      {
////        std::unique_lock<std::mutex> l(versions_mtx);// TODO(ruihong):
///remove all the superversion mutex usage.
//        c->ReleaseInputs();
//        {
...
//      //      write_stall_mutex_.AssertNotHeld();
//      // Only when there is enough input level files and output level files
//      will the subcompaction triggered if (usesubcompaction &&
//      c->num_input_files(0)>=4 && c->num_input_files(1)>1){
...
//             compact->compaction->num_input_files(0),compact->compaction->num_input_files(1)
//             );
////#endif
//      DEBUG("Non-trivalcompaction!\n");
//      std::cout << "compaction task table number in the first
//      level"<<compact->compaction->inputs_[0].size() << std::endl; if
//      (!status.ok()) {
...
void Memory_Node_Keeper::PersistSSTables(void* arg) {
  ...
  // The version edit merger has merge enough edits, lets make those files
  // durable.
  DEBUG("A work pesistent work request was executed--------------\n");
  assert(edit_merger->GetNewFilesNum() > 0);
  int thread_number =
      edit_merger->GetNewFilesNum() - edit_merger->only_trival_change.size();
#ifndef NDEBUG
  // assert all the file number in the only trival change list exist in the
  // ve_merger
  //  so that we can calculate the thread number correctly.
  for (auto iter : edit_merger->only_trival_change) {
    assert(edit_merger->GetNewFiles()->find(iter) !=
           edit_merger->GetNewFiles()->end());
  }
#endif
  ...
  for (auto iter : *edit_merger->GetNewFiles()) {
    ...
    if (edit_merger->only_trival_change.find(iter.first) ==
        edit_merger->only_trival_change.end()) {
      threads[i] =
          std::thread(&Memory_Node_Keeper::PersistSSTable, this, iter.second);
      i++;
    }
  }
  ...
  if (descriptor_log == nullptr) {
    ...
    new_manifest_file =
        DescriptorFileName("./db_content", manifest_file_number_);
...
void Memory_Node_Keeper::PersistSSTable(
    std::shared_ptr<RemoteMemTableMetaData> sstable_ptr) {
...
void Memory_Node_Keeper::UnpinSSTables_RPC(VersionEdit_Merger* edit_merger,
  ...
  for (auto iter : *edit_merger->GetNewFiles()) {
    ...
    DEBUG_arg("Unpin file number is %lu, id 1 ****************\n",
              iter.second->number);
    assert(edit_merger->debug_map.find(iter.second->number) ==
           edit_merger->debug_map.end());
...
Status Memory_Node_Keeper::DoCompactionWork(CompactionState* compact,
  ...
  Iterator* input =
      versions_->MakeInputIteratorMemoryServer(compact->compaction);
  ...
  while (input->Valid()) {
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (!has_current_user_key) {
        // TODO: can we avoid the data copy here, can we set two buffers in
        // block and make
        //  the old user key not be garbage collected so that the old Slice can
...
Status Memory_Node_Keeper::DoCompactionWorkWithSubcompaction(
  ...
  // TODO need to check the snapeshot in the compute node. Or modify the logic
  // in get()
  ...
  if (boundaries->size() <= opts->MaxSubcompaction) {
    ...
    for (int i = 0; i < sizes->size(); i++) {
      if ((*sizes)[i] <= opts->max_file_size / 4) small_files.push_back(i);
    }
    int big_files_num = boundaries->size() - small_files.size();
    int files_per_subcompaction =
        big_files_num / opts->MaxSubcompaction +
    ...
    for (size_t i = 0; i <= boundaries->size(); i++) {
      ...
      Slice* start = i == 0 ? nullptr : &(*boundaries)[i - 1];
      int files_counter =
          range_size <= opts->max_file_size / 4 ? 0 : 1;  // count this file.
  ...
  for (size_t i = 1; i < compact->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&Memory_Node_Keeper::ProcessKeyValueCompaction,
                             this, &compact->sub_compact_states[i]);
  }
...
void Memory_Node_Keeper::ProcessKeyValueCompaction(
  ...
  Iterator* input =
      versions_->MakeInputIteratorMemoryServer(sub_compact->compaction);

  // Release mutex while we're actually doing the compaction work
  //  undefine_mutex.Unlock();

  if (start != nullptr) {
    // The compaction range is (start, end]. so we set 0 as look up key
    // sequence.
    ...
    // The first key larger or equal to start_internal was covered in the
    // subtask before it. The range for the subcompactions are (s1,e1] (s2,e2]
    // ... (sn,en]
    input->Next();
  } else {
    input->SeekToFirst();
  ...
  while (input->Valid()) {
    ...
    if (!drop) {
      ...
      if (sub_compact->builder->FileSize() >=
        ...
        assert(internal_comparator_.Compare(
                   sub_compact->current_output()->largest,
...
Status Memory_Node_Keeper::OpenCompactionOutputFile(
    SubcompactionState* compact) {
  ...
  if (s.ok()) {
    ...
    compact->builder = new TableBuilder_BAMS(*opts, Compact, rdma_mg);
#endif
  }
  return s;
}
...
Status Memory_Node_Keeper::FinishCompactionOutputFile(
    SubcompactionState* compact, Iterator* input) {
  assert(compact != nullptr);
  //  assert(compact->outfile != nullptr);
  assert(compact->builder != nullptr);
  ...
  printf("File number %lu largest key size is %lu",
...
Status Memory_Node_Keeper::InstallCompactionResults(CompactionState* compact,
  ...
  if (compact->sub_compact_states.size() == 0) {
    for (size_t i = 0; i < compact->outputs.size(); i++) {
      ...
      // Verify that the table is usable
      Iterator* it =
          versions_->table_cache_->NewIterator_MemorySide(ReadOptions(), meta);
      //        s = it->status();
...
void Memory_Node_Keeper::server_communication_thread(std::string client_ip,
  ...
  // TODO: use Local_Memory_Allocation to bulk allocate, and assign within this
  // function.
  //    ibv_mr send_mr[32] = {};
  //    for(int i = 0; i<32; i++){
  ...
  //    char* send_buff;
  //    if (!rdma_mg_->Local_Memory_Register(&send_buff, &send_mr, 1000,
  //    std::string())) {
  ...
  //    char* recv_buff;
  //    if (!rdma_mg_->Local_Memory_Register(&recv_buff, &recv_mr, 1000,
  //    std::string())) {
  //      fprintf(stderr, "memory registering failed by size of 0x%x\n", 1000);
  //    }
  //  post_receive<int>(recv_mr, client_ip);
  for (int i = 0; i < R_SIZE; i++) {
    rdma_mg->post_receive<RDMA_Request>(&recv_mr[i], compute_node_id,
                                        client_ip);
  }
  //    rdma_mg_->post_receive(recv_mr, client_ip,
  //    sizeof(Computing_to_memory_msg));
  ...
  if (rdma_mg->sock_sync_data(
          socket_fd, 1, temp_send,
  ...
  if (rdma_mg->connection_counter.load() == rdma_mg->compute_nodes.size() &&
      rdma_mg->node_id == 0) {
    std::thread thread_sync(&RDMA_Manager::sync_with_computes_Mside,
                            rdma_mg.get());
    // Need to be detached.
    thread_sync.detach();
  }
  ...
  //  ntohl(receive_msg_buf->content.qp_config.qp_num);
  //  receive_msg_buf->content.qp_config.lid =
  //  ntohs(receive_msg_buf->content.qp_config.lid); ibv_wc wc[3] = {};
  ...
  while (true) {
    ...
    *receive_msg_buf = *(RDMA_Request*)recv_mr[buffer_position].addr;
    ...
    if (receive_msg_buf->command == create_mr_) {
      ...
      //        rdma_mg_->post_send<ibv_mr>(send_mr,client_ip);  // note here
      //        should be the mr point to the send buffer.
      ...
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[buffer_position],
                                          compute_node_id, client_ip);
      // TODO: implement a durable bg thread and make a shadow verison set if
      // possible.

      install_version_edit_handler(receive_msg_buf, client_ip, compute_node_id);
    } else if (receive_msg_buf->command == near_data_compaction) {
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[buffer_position],
                                          compute_node_id, client_ip);
      Arg_for_handler* argforhandler =
          new Arg_for_handler{.request = receive_msg_buf,
                              .client_ip = client_ip,
                              .target_node_id = compute_node_id};
      BGThreadMetadata* thread_pool_args =
          new BGThreadMetadata{.db = this, .func_args = argforhandler};
      Compactor_pool_.Schedule(&Memory_Node_Keeper::RPC_Compaction_Dispatch,
                               thread_pool_args);
      ...
      Message_handler_pool_.Schedule(
          &Memory_Node_Keeper::RPC_Garbage_Collection_Dispatch,
          thread_pool_args);
      ...
      // THis should not be called because the recevei mr will be reset and the
      // buffer
      //  counter will be reset as 0
      rdma_mg->post_receive<RDMA_Request>(&recv_mr[buffer_position],
                                          compute_node_id, client_ip);
      qp_reset_handler(receive_msg_buf, client_ip, socket_fd, compute_node_id);
      DEBUG("QP has been reconnect from the memory node side\n");
      // TODO: Pause all the background tasks because the remote qp is not
      // ready.
      //  stop sending back messasges. The compute node may not reconnect its qp
      //  yet!
    } else {
  ...
  assert(false);
  // TODO: Build up a exit method for shared memory side, don't forget to
  // destroy all the RDMA resourses.
}
  ...
  for (iterator = resolved_addr; iterator; iterator = iterator->ai_next) {
    ...
    if (sockfd >= 0) {
      ...
      while (1) {
        ...
        // No need to detach, because the main_comm_threads will not be
        // destroyed.
...
void Memory_Node_Keeper::create_mr_handler(RDMA_Request* request,
  ...
  {
    std::unique_lock<std::shared_mutex> lck(rdma_mg->local_mem_mutex);
    assert(request->content.mem_size =
               1024 * 1024 * 1024);  // Preallocation requrie memory is 1GB
    if (!rdma_mg->Local_Memory_Register(&buff, &mr, request->content.mem_size,
                                        Default)) {
      fprintf(stderr, "memory registering failed by size of 0x%x\n",
              static_cast<unsigned>(request->content.mem_size));
    }
    //      printf("Now the Remote memory regularated by compute node is %zu
    //      GB",
...
void Memory_Node_Keeper::create_qp_handler(RDMA_Request* request,
  ...
  registered_qp_config* remote_con_data =
      new registered_qp_config(request->content.qp_config);
...
void Memory_Node_Keeper::install_version_edit_handler(RDMA_Request* request,
  ...
  DEBUG_arg("Version edit decoded, new file number is %zu",
            version_edit->GetNewFilesNum());
//  std::unique_lock<std::mutex> lck(versionset_mtx, std::defer_lock);
...
#endif
      Arg_for_persistent* argforpersistence =
          new Arg_for_persistent{.edit_merger = ve_m, .client_ip = client_ip};
      BGThreadMetadata* thread_pool_args =
          new BGThreadMetadata{.db = this, .func_args = argforpersistence};
...
void Memory_Node_Keeper::sst_garbage_collection(void* arg) {
  ...
  // know the size of the
  //  version edit in the first REQUEST from compute node.
  volatile uint64_t* polling_byte =
      (uint64_t*)((char*)large_recv_mr.addr + request->content.gc.buffer_size -
                  sizeof(uint64_t));
  *polling_byte = 0;
  asm volatile("sfence\n" : :);
  asm volatile("lfence\n" : :);
  ...
  //      rdma_mg->Deallocate_Local_RDMA_Slot(large_send_mr.addr,
  //      "version_edit");
  delete request;
...
void Memory_Node_Keeper::sst_compaction_handler(void* arg) {
  ...
  //    volatile char* polling_byte = (char*)large_recv_mr.addr +
  //    request->content.sstCompact.buffer_size - 1; memset((void*)polling_byte,
  //    0, 1); asm volatile ("sfence\n" : : ); asm volatile ("lfence\n" : : );
  //    asm volatile ("mfence\n" : : );
  rdma_mg->RDMA_Write(remote_prt, remote_rkey, &send_mr, sizeof(RDMA_Reply),
                      client_ip, IBV_SEND_SIGNALED, 1);
#endif

  ibv_mr remote_mr;
  remote_mr.addr = remote_large_prt;
  remote_mr.rkey = remote_large_rkey;
  // NOte we have to use the polling mechanism because other wise the read can
  // be finished ealier
  //  than we want.
  volatile char* polling_byte =
      (char*)large_recv_mr.addr + request->content.sstCompact.buffer_size - 1;
  memset((void*)polling_byte, 0, 1);
  asm volatile("sfence\n" : :);
  asm volatile("lfence\n" : :);
  asm volatile("mfence\n" : :);
  ...
  rdma_mg->RDMA_Read(&remote_mr, &large_recv_mr,
                     request->content.sstCompact.buffer_size + 1, client_ip, 0,
                     0, target_node_id);
  size_t counter = 0;
  // polling the finishing bit for compaction task transmission.
  while (*(unsigned char*)polling_byte == 0) {
  ...
  // Decode compaction
  c.DecodeFrom(
      Slice((char*)large_recv_mr.addr, request->content.sstCompact.buffer_size),
      1);
  // Note need to check whether the code below is correct.
  //    void* remote_large_recv_ptr =  *(void**)new_pos.data();
  //    uint32_t remote_large_recv_rkey = *(uint32_t*)(new_pos.data() +
  //    sizeof(void*));
  ...
  CompactionState* compact = new CompactionState(&c);
  if (usesubcompaction && c.num_input_files(0) >= 4 &&
...
//     memset((char*)large_send_mr.addr, 1, 1);
  ...
  *(size_t*)send_mr.addr = serilized_ve.size() + 1;
  ...
  // Prepare the receive buffer for the version edit duribility and the file
  // numbers.
  ...
  asm volatile("lfence\n" : :);
  asm volatile("mfence\n" : :);
  rdma_mg->RDMA_Write_Imme(remote_large_prt, remote_large_rkey, &large_send_mr,
                           serilized_ve.size() + 1, client_ip,
                           IBV_SEND_SIGNALED, 1, imm_num, target_node_id);
#ifndef NDEBUG
  debug_counter.fetch_add(1);
#endif
//    rdma_mg->RDMA_Write(remote_large_prt, remote_large_rkey,
//                             &large_send_mr, serilized_ve.size() + 1,
//                             client_ip, IBV_SEND_SIGNALED, 1);
#ifdef WITHPERSISTENCE
  int counter = 0;
...
#endif
  // The validation below should happen before the real edit send back to the
  // compute node
  //  because there is no reference for this test, the sstables can be garbage
  //  collected.
  // #ifndef NDEBUG
  //     for(auto pair : *compact->compaction->edit()->GetNewFiles()){
  //       // Verify that the table is usable
  //       Iterator* it =
  //       versions_->table_cache_->NewIterator_MemorySide(ReadOptions(),
  //       pair.second);
  //       //        s = it->status();
  //
  //       it->SeekToFirst();
...
void Memory_Node_Keeper::qp_reset_handler(RDMA_Request* request,
                                          std::string& client_ip, int socket_fd,
                                          uint8_t target_node_id) {
  ibv_mr send_mr;
  char temp_receive[2];
  char temp_send[] = "Q";
  ...
  // NOte: This is not correct because we did not recycle the receive mr, so we
  // also
  //   need to repost the receive wr and start from mr #1
  //    send_pointer->received = true;
  //    rdma_mg->RDMA_Write(request.reply_buffer, request.rkey,
  //                        &send_mr, sizeof(RDMA_Reply),client_ip,
  //                        IBV_SEND_SIGNALED,1);
  rdma_mg->sock_sync_data(socket_fd, 1, temp_send, temp_receive);
  delete request;
  //    rdma_mg->Deallocate_Local_RDMA_Slot(send_mr.addr, "message");
}
...
void Memory_Node_Keeper::Edit_sync_to_remote(
  ...
  if (edit->IsTrival()) {
    ...
    uint64_t file_number;
    uint8_t node_id;
    edit->GetTrivalFile(level, file_number, node_id);
    ...
    *receive_pointer = {};
    ...
    printf(
        "Request was sent, sub version id is %lu, polled buffer address is %p, "
        "checkbyte is %d\n",
    ...
    //  does not matter in our case because we do not care when will the message
