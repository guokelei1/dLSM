...
PosixEnv::PosixEnv()
  ...
  // Initialize the rdma manager, the remote block size will be configured in
  // the beggining.
  ...
  // Unlike the remote block size, the local block size is adjustable, and there
  // could be different local memory pool with different size. each size of
  // memory pool will have an ID below is "4k"

  // client will try to connect to the remote memory, now there is only one
  // remote memory.
...
void PosixEnv::Schedule(
  ...
  // TOTHINK: why there is a lock limiting one thread to do the compaction at
  // the same time
  //  Can we simply create a pool of threads and then every time we signal all
  //  the pool threads, the thread check whether there is available job to do.
  //  There are two options:
  //  1.The scheduled job should not picked by the thread itself (eg which table
  //  to flush or which tables to compact), the job should be confirmed out side
  //  the thread.
  //  2. the thread itself pick the job and that needs a mutex, or lock free
  //  method. potentially you can decide the job when you are inserting the job
  //  to the queue. so that there is no lock inside the thread execution.
...
void PosixEnv::Schedule(
  ...
  switch (type) {
    ...
    case CompactionThreadPool:
      ...
      //      subcompaction.Schedule(background_work_function,
      //      background_work_arg); break;
  }
}
