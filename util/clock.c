...
void* (*actions[16])(
    void* arg);  // the registered actions to perform repeatedly

void* task_a(void* nil) {
  printf("A\n");
  return NULL;
}  // example tasks to repeat
void* task_b(void* nil) {
  printf("B\n");
  return NULL;
}  // -----------------------
void* task_c(void* nil) {
  printf("C\n");
  return NULL;
}  // -----------------------
...
int main() {
  ...
  // Add the things you want to repeat every so ofter (must be less than 1
  // second)
  repeat(task_a, 750);
  repeat(task_b, 500);
  repeat(task_c, 450);
  ...
  for (;;) {
    int ready;  // number of descriptors (not all necessarily timers) that are
                // ready for read

    if ((ready = epoll_wait(epoll_fd, events, 16, -1)) < 0) continue;
